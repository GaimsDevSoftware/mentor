#!/bin/bash
# Build a downloadable macOS launcher app + .dmg for Mentor.
#
#   ./build-macos-app.sh
#
# Produces:
#   dist/Mentor.app     — double-click: starts the local server (using this
#                         repo's venv) and opens the UI in an app-style window.
#   dist/Mentor.dmg     — drag-to-Applications disk image (the downloadable).
#
...
# Icon (best effort) — use the Mentor icon when present, otherwise fall back to Odysseus art.
ICON_SOURCE="$REPO_DIR/designs/mentor-icon-512.png"
[ -f "$ICON_SOURCE" ] || ICON_SOURCE="$REPO_DIR/docs/odysseus.jpg"
if [ -f "$ICON_SOURCE" ] && command -v sips >/dev/null 2>&1; then
  ...
  sips -c 720 720 "$ICON_SOURCE" --out "$TMPIMG/sq.png" >/dev/null 2>&1 || cp "$ICON_SOURCE" "$TMPIMG/sq.png"
  sips -z 512 512 "$TMPIMG/sq.png" --out "$TMPIMG/icon.png" >/dev/null 2>&1
  if sips -s format icns "$TMPIMG/icon.png" --out "$APP/Contents/Resources/mentor.icns" >/dev/null 2>&1; then
    echo "  icon:        mentor.icns"
  else
    echo "  icon:        (skipped — conversion failed)"
  fi
  rm -rf "$TMPIMG"
else
  echo "  icon:        (skipped — no icon source)"
fi
...
<dict>
    ...
    <key>CFBundleIdentifier</key>      <string>com.gaimsdevsoftware.mentor.launcher</string>
    <key>CFBundleVersion</key>         <string>1.0</string>
    <key>CFBundleShortVersionString</key><string>1.0</string>
    <key>CFBundlePackageType</key>     <string>APPL</string>
    <key>CFBundleExecutable</key>      <string>$APP_NAME</string>
    <key>CFBundleIconFile</key>        <string>mentor</string>
...
#!/bin/bash
# Mentor.app — start the local server and open the UI in an app window.
...
LOG="$INSTALL_DIR/logs/mentor-app.log"

notify() { /usr/bin/osascript -e "display notification \"$1\" with title \"Mentor\"" >/dev/null 2>&1; }
die_gui() {
  /usr/bin/osascript -e "display dialog \"$1\" with title \"Mentor\" buttons {\"OK\"} default button 1 with icon stop" >/dev/null 2>&1
  exit 1
}

[ -x "$UVICORN" ] || die_gui "Mentor isn't set up yet. Open Terminal and run:
...
for i in $(seq 1 120); do
  ...
  kill -0 "$SERVER_PID" 2>/dev/null || die_gui "Mentor failed to start. Log:
$LOG"
  sleep 1
done
...
if [ "$READY" = "1" ]; then
  ...
  notify "Mentor is taking a while — open $URL once it finishes starting."
fi
wait "$SERVER_PID"
LAUNCHER
...
echo "Install:       open '$DIST/$APP_NAME.dmg'  (drag Mentor to Applications)"
