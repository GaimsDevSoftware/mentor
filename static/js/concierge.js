...
(function () {
  ...
  async function act(a) {
    ...
    try {
      ...
      if (t === 'auto_roles') { var rr = await J('/api/setup/auto-roles', { method: 'POST' }); if (!rr.ok) return rr.error || 'could not auto-fill roles'; var aa = rr.assigned || {}; return 'roles filled — ' + Object.keys(aa).map(function (k) { return k + '=' + aa[k]; }).join(', ') + (rr.vision ? ' · vision ok' : ' · no vision model'); }
