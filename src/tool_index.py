...
BUILTIN_TOOL_DESCRIPTIONS: Dict[str, str] = {
    ...
    "trigger_research": "Start a deep research job on any topic — appears in the Deep Research sidebar, streams progress, produces a detailed report. Use for 'research X', 'look into Y', 'do deep research on Z', 'investigate'. NOT a scheduled task — it runs now and surfaces in the sidebar. To run it with CLAUDE'S live web research instead of the local model (higher quality), pass engine=\"claude\" — use this when the user says 'research X with Claude' / 'use Claude to research'. The Claude run lands in the Deep Research Library when done.",
    "self_coder": "Propose, list, apply, or discard autonomous code changes to this app itself. The self-coder uses Aider to make changes on an isolated git branch, verifies them (compile + boot check + optional tests), and optionally auto-merges safe plugin-only changes. Use for 'fix this bug in the app', 'improve the code', 'self-repair', 'propose a code change', 'list code proposals', 'apply proposal X'. Actions: propose (instruction + optional files), list, get (id), apply (id), discard (id), status.",
}
