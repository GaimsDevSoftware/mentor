#!/bin/bash
# Mentor — one-command quick start for macOS (Apple Silicon).
#
#   ./start-macos.sh
#
# Installs everything Mentor needs via Homebrew, sets up a local Python
...
# that's already done.
#
# Why native (not Docker): Cookbook serves models on whatever machine Mentor
...
echo "▶ Mentor quick start for macOS"
    ...
    (
        for _ in $(seq 1 90); do
            if (exec 3<>"/dev/tcp/$PROBE_HOST/$PORT") 2>/dev/null; then
                printf '\n'
                printf '  ┌────────────────────────────────────────────┐\n'
                printf '  │  ✓ Mentor is ready — opening your browser    │\n'
                printf '  │     %-40s │\n' "$URL"
...
echo
echo "▶ Starting Mentor — it will open in your browser at $URL"
