...
def analyze_model(model, system, target_quant=None, scoring_use_case=None, target_context=None):
    ...
    # Multi-GPU filter: skip the row if an AUTO-RESOLVED quant is a GGUF tier
    # (Q*/IQ-prefixed) — vLLM/SGLang can't serve those, so defaulting to one on
    # a 2+ GPU rig just clutters the list with unservable candidates. But when
    # the user EXPLICITLY asked for a specific Q-tier (target_quant), honor it:
    # surface the row (llama.cpp serves GGUF on a single GPU, often too_tight)
    # rather than silently dropping the choice they made.
    if (
        not target_quant
        and gpu_count >= 2
        and quant_to_try
